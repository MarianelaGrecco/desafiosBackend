...
export const addItemToCart = async (req, res) => {
  ...
  try {
    ...
    if (user.role === "premium") {
      ...
      const ownedProducts = user.products.map((product) => product.toString());
      if (ownedProducts.includes(productId)) {
        logger.warn("Premium user cannot add owned product to cart:", userId, productId);
        return res.status(403).json({ message: "Premium user cannot add owned product to cart" });
      }
    }
